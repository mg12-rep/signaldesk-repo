...
class Symbol(Base):
    ...
    eod_data = relationship(
        "MarketDataEOD", back_populates="symbol", cascade="all, delete-orphan"
    )
    history_data = relationship(
        "MarketDataHistory", back_populates="symbol", cascade="all, delete-orphan"
    )
...
class Order(Base):
    ...
    broker = Column(String(50), nullable=False)  # ZERODHA, UPSTOX, IBKR
    strategy = Column(
...
class Trade(Base):
    ...
    )  # STOP, PROFIT_TARGET_20, SMA50_EXIT
